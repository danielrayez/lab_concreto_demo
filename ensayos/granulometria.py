...
with col3:
    try:
        df = pd.read_csv(r'C:\Lab_concreto_beta\utils\gram_limites.txt')
        agg = st.selectbox("Selecciona el tipo de agregado", options=df['material'].unique(), index=0, key='material_select')
        df_agg = df[df['material'] == agg].copy()
